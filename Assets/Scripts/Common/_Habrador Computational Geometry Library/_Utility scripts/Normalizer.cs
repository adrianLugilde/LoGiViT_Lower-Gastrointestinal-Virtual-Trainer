...
namespace Habrador_Computational_Geometry
{
    ...
    public class Normalizer2
    {
        ...
        //LUGILDE
        public MyVector2 Normalize_3DY(MyVector2 point)
        {
            float x = (point.x - boundingBox.min.x) / dMax;
            float y = (point.y - boundingBox.min.y) / dMax;

            MyVector2 pNormalized = new MyVector2(x, y, point._3DY);

            return pNormalized;
        }

        //LUGILDE
        public List<MyVector2> Normalize_3DY(List<MyVector2> points)
        {
            List<MyVector2> normalizedPoints = new List<MyVector2>();

            foreach (MyVector2 p in points)
            {
                normalizedPoints.Add(Normalize_3DY(p));
            }
        ...
        //LUGILDE
        public MyVector2 UnNormalize_3DY(MyVector2 point)
        {
            float x = (point.x * dMax) + boundingBox.min.x;
            float y = (point.y * dMax) + boundingBox.min.y;

            MyVector2 pUnNormalized = new MyVector2(x, y, point._3DY);
        ...
        //LUGILDE
        public HalfEdgeData2 UnNormalize_3DY(HalfEdgeData2 data)
        {
            foreach (HalfEdgeVertex2 v in data.vertices)
            {
                MyVector2 vUnNormalized = UnNormalize_3DY(v.position);
