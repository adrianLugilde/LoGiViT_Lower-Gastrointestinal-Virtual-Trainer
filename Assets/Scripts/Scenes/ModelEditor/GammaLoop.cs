...
{
    [System.Serializable]
    public class GammaLoop : BaseLoop
    {
        [Header("Gamma Loop Parameters")]
        ...
        private void GenerateCircleLoop(Spline spline, int startNodeIndex, int endNodeIndex, int segmentNodeCount, SegmentCoordinateSystem loopCoordinateSystem)
        {
            ...
            // Define the helix axes
            Vector3 greenAxis = loopCoordinateSystem.segmentDirection;  // Y - helix circular plane component
            Vector3 blueAxis = loopCoordinateSystem.segmentUp;          // Z - helix circular plane component  
            Vector3 redAxis = loopCoordinateSystem.segmentRight;        // X - helix progression axis
            ...
            {
                float greenRotationRad = tiltAroundUp * Mathf.Deg2Rad;
            ...
            {
                float blueRotationRad = tiltAroundFlow * Mathf.Deg2Rad;
            ...
            for (int i = 0; i < loopNodeCount; i++)
            {
                ...
                Vector3 circularComponent = greenAxis * (radius * Mathf.Cos(angle)) + 
                                          blueAxis * (radius * Mathf.Sin(angle));
                ...
                // Tangent combines circular tangent and linear progression
                Vector3 circularTangent = greenAxis * (-radius * Mathf.Sin(angle)) + 
                                        blueAxis * (radius * Mathf.Cos(angle));
                ...
                // Keep the original up vector or use the loop coordinate system's up
        ...
        public override void DrawGizmos(Spline spline)
        {
            ...
            DrawGammaLoopPreview(segmentCoordinateSystem, loopCoordinateSystem);
        }

        private void DrawGammaLoopPreview(SegmentCoordinateSystem segmentCoordinateSystem, SegmentCoordinateSystem loopCoordinateSystem)
        {
            ...
            // Displace downward in the segment coordinate system's "up" axis (controlled by depth parameter)
            Vector3 gammaPosition = basePosition - (segmentCoordinateSystem.segmentUp * depth);
            
            // Create a copy of the loop center displaced down along the green axis (loop direction) by the depth value
            // and offset laterally along the red axis
            Vector3 loopCenter = loopCoordinateSystem.segmentCenter;
            Vector3 loopCenterCopy = loopCenter - (loopCoordinateSystem.segmentDirection * depth) + (loopCoordinateSystem.segmentRight * lateralOffset);
            
            // Draw the gamma loop gizmo
            Gizmos.color = Color.yellow;
            Gizmos.DrawWireSphere(gammaPosition, 0.1f);
            
            // Draw a line from base position to displaced position to show the displacement
            Gizmos.color = Color.cyan;
            Gizmos.DrawLine(basePosition, gammaPosition);
            
            // Draw the loop center copy
            Gizmos.color = Color.magenta;
            Gizmos.DrawWireSphere(loopCenterCopy, 0.08f);
            
            // Draw a line from original loop center to the copy to show the displacement along green axis
            Gizmos.color = Color.green;
            Gizmos.DrawLine(loopCenter, loopCenterCopy);
            
            // Draw circle of nodes around the displaced center in the green-blue plane
            DrawCircleOfNodes(loopCenterCopy, loopCoordinateSystem);
            
            // Draw segment coordinate system for reference (larger, more transparent)
            DrawCoordinateSystem(segmentCoordinateSystem, "Segment", Color.white, 0.4f, 0.5f);
        ...
        private void DrawCircleOfNodes(Vector3 center, SegmentCoordinateSystem loopCoordinateSystem)
        {
            ...
            // Define the helix axes
            Vector3 greenAxis = loopCoordinateSystem.segmentDirection; // Y - circular plane component
            Vector3 blueAxis = loopCoordinateSystem.segmentUp;         // Z - circular plane component
            Vector3 redAxis = loopCoordinateSystem.segmentRight;       // X - helix progression axis
            ...
            if (Mathf.Abs(tiltAroundUp) > 0.01f || Mathf.Abs(tiltAroundFlow) > 0.01f)
            {
                ...
                Gizmos.color = Color.green;
                Gizmos.DrawLine(center, center + greenAxis * axisLength);
                Gizmos.color = Color.blue;
                Gizmos.DrawLine(center, center + blueAxis * axisLength);
                Gizmos.color = Color.red;
                Gizmos.DrawLine(center, center + redAxis * axisLength);
            }
            
            // Draw circular plane outline at center for reference
            Gizmos.color = Color.gray;
            Vector3[] circlePoints = new Vector3[12];
            for (int i = 0; i < 12; i++)
            {
                float angle = (2f * Mathf.PI * i) / 12;
                Vector3 localPosition = greenAxis * (radius * Mathf.Cos(angle)) + 
                                       blueAxis * (radius * Mathf.Sin(angle));
