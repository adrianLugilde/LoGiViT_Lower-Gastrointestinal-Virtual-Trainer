...
{
    [System.Serializable]
    public class ReversedAlphaLoop : BaseLoop
    {
        [Header("Reversed Alpha Loop Parameters")]
        ...
        {
            return "Reversed Alpha Loop";
        }

        public ReversedAlphaLoop()
        {
            // N-loops specifically target the sigmoid segment
            targetSegment = LI_Segment.Sigmoid;
            loopName = "Reversed Alpha Loop";
            
            // Override position in segment range for reversed alpha loop
            positionInSegment = 0.675f; // Default to middle of extended range [0, 1.35]
        }
        ...
        private void GenerateCircleLoop(Spline spline, int startNodeIndex, int endNodeIndex, int segmentNodeCount, SegmentCoordinateSystem loopCoordinateSystem)
        {
            ...
            Vector3 displacedCenter = loopCenter - (loopCoordinateSystem.segmentUp * depth) + (loopCoordinateSystem.segmentRight * lateralOffset);
            ...
            for (int i = 0; i < loopNodeCount; i++)
            {
                ...
                // Calculate position in helix (blue-green circular plane)
        ...
        public override void DrawGizmos(Spline spline)
        {
            ...
            DrawReversedAlphaLoopPreview(segmentCoordinateSystem, loopCoordinateSystem);
        }

        private void DrawReversedAlphaLoopPreview(SegmentCoordinateSystem segmentCoordinateSystem, SegmentCoordinateSystem loopCoordinateSystem)
        {
            ...
            Vector3 alphaPosition = basePosition - (segmentCoordinateSystem.segmentUp * depth);
            
            // Create a copy of the loop center displaced down along the blue axis (up axis) by the depth value
            // and offset laterally along the red axis
            Vector3 loopCenter = loopCoordinateSystem.segmentCenter;
            Vector3 loopCenterCopy = loopCenter - (loopCoordinateSystem.segmentUp * depth) + (loopCoordinateSystem.segmentRight * lateralOffset);
