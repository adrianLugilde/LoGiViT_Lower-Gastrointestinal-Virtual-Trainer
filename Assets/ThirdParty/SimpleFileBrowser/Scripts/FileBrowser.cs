...
namespace SimpleFileBrowser
{
	...
	{
		public int currentDisplayMode = 0;
...
#pragma warning restore 0649
		...
		private static FileBrowser Instance
		{
			get
			{
                var tempDisplayMode = PlayerPrefs.GetInt("DisplayMode");
                if( !m_instance )
				{
                    //m_instance = Instantiate(Resources.Load<GameObject>(tempDisplayMode == 0 ? "SimpleFileBrowserCanvas" : "SimpleFileBrowserMainMenuCanvasVR")).GetComponent<FileBrowser>();
					if(tempDisplayMode == 0 )
					{
						m_instance = Instantiate(Resources.Load<GameObject>("SimpleFileBrowserCanvas").GetComponent<FileBrowser>());
						DontDestroyOnLoad( m_instance );
					} else
					{
						m_instance = GameObject.FindWithTag("FileBrowser").GetComponent<FileBrowser>();
					}
                    //DontDestroyOnLoad( m_instance.gameObject );
					m_instance.gameObject.SetActive( false );
                    m_instance.currentDisplayMode = tempDisplayMode;
                }/*else if (m_instance)
                {
                    if (tempDisplayMode != m_instance.currentDisplayMode)
                    {
                        Destroy(m_instance.gameObject);
                        m_instance = Instantiate(Resources.Load<GameObject>(tempDisplayMode == 0 ? "SimpleFileBrowserCanvas" : "SimpleFileBrowserCanvasVR")).GetComponent<FileBrowser>();
                        DontDestroyOnLoad(m_instance.gameObject);
                        m_instance.gameObject.SetActive(false);
                        m_instance.currentDisplayMode = tempDisplayMode;
                    }
