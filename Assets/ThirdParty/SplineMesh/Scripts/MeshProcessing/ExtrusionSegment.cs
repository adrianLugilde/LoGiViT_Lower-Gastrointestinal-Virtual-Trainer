...
namespace SplineMesh {
    ...
    public class ExtrusionSegment : MonoBehaviour {
        ...
        public void Compute() {
            ...
            var bentVertices = new List<GeometryUtils.Vertex>(vertsInShape * 2 * segmentCount * 3);

            foreach (var sample in path) {
                foreach (ExtrusionSegment.Vertex v in shapeVertices) {
                    bentVertices.Add(sample.GetBent(new GeometryUtils.Vertex(
            ...
            GeometryUtils.MeshUtils.CopyMesh(mf.sharedMesh,
                mf.sharedMesh,
                false,
                triangleIndices,
