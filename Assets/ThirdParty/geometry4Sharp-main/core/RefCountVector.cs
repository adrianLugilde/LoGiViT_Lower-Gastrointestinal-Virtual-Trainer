...
namespace g4
{
    ...
    /// 
    /// **refcounts are ints** so the maximum count is 65536. 
    ...
    public class RefCountVector : System.Collections.IEnumerable
    {
        ...
        DVector<int> ref_counts;
        DVector<int> free_indices;
        int used_count;

        public RefCountVector()
        {
            ref_counts = new DVector<int>();
        ...
        {
            ref_counts = new DVector<int>(copy.ref_counts);
            free_indices = new DVector<int>(copy.free_indices);
            used_count = copy.used_count;
        }

        public RefCountVector(int[] raw_ref_counts, bool build_free_list = false)
        {
            ref_counts = new DVector<int>(raw_ref_counts);
            free_indices = new DVector<int>();
            used_count = 0;
            if (build_free_list)
                rebuild_free_list();
        ...
        public DVector<int> RawRefCounts {
        ...
        public int increment(int index, int increment = 1) {
            ...
            Util.gDevAssert(  (int)(ref_counts[index] + increment) > 0 );
            ref_counts[index] += increment;
            return ref_counts[index];       
        }

        public void decrement(int index, int decrement = 1) {
            Util.gDevAssert( isValid(index) );
        ...
        public void set_Unsafe(int index, int count)
        {
