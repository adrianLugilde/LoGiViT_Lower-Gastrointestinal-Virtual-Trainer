...
| **Inteligencia Artificial** | Local (Ollama: compatible con cualquier modelo como Llama 3.2, Mistral, BioMistral, Phi), Cloud (OpenAI GPT-4o-mini, Whisper, TTS-1) y Google Cloud Text-to-Speech |
...
- **Inteligencia Artificial y Audio:**
  - [OllamaClient.cs](Assets/Scripts/Scenes/SceneCommons/OllamaClient.cs): Cliente HTTP REST asíncrono para comunicarse con un servidor local de Ollama (compatible con cualquier modelo descargado, por defecto `llama3.2`).
...
- Para uso de IA en local: Instalar y ejecutar [Ollama](https://ollama.ai/) con cualquier modelo de tu elección (ej. `ollama run llama3.2`, `mistral`, `biomistral`, `phi4`).
...
#### Opción C: Usar Ollama en Local (Totalmente Gratuito, Offline y con Cualquier Modelo)
...
> [!NOTE]
> No estás limitado a `llama3.2`. Puedes usar cualquier modelo soportado por Ollama (como `mistral`, `biomistral`, `phi4`, `qwen2.5`, `meditron`, etc.). `llama3.2` se incluye simplemente como valor predeterminado ligero (1B/3B) recomendado para ejecutarse de forma fluida en equipos convencionales en paralelo con Unity en VR sin saturar la CPU/GPU.

1. Instala [Ollama](https://ollama.ai/) en tu máquina y descarga el modelo deseado en la terminal:
   ```bash
   # Modelo ligero por defecto:
   ollama run llama3.2
...
2. En Unity, abre la sala de entrenamiento correspondiente y selecciona el componente `OllamaClient`:
   - Configura el campo **`Llm`** con el nombre exacto del modelo que tengas descargado (ej. `llama3.2`, `mistral` o `biomistral`).
...
- **Error: "Ollama connection refused" o timeout en el puerto 11434:**
  ...
  - *Solución:* Abre una terminal en Windows y ejecuta `ollama serve` o verifica que el modelo esté descargado con `ollama list`. Si no está, ejecuta `ollama pull <nombre_modelo>` (ej. `ollama pull llama3.2`).
