...
| **Artificial Intelligence** | Local (Ollama - Llama 3.2), Cloud (OpenAI GPT-4o-mini, Whisper, TTS-1) and Google Cloud Text-to-Speech |
...
- **Artificial Intelligence and Audio:**
  - [OllamaClient.cs](Assets/Scripts/Scenes/SceneCommons/OllamaClient.cs): Asynchronous REST HTTP client to communicate with a local Ollama server (`llama3.2`).
...
- For local AI execution: Install and run [Ollama](https://ollama.ai/) with `llama3.2` (`ollama run llama3.2`).
...
#### Option C: Running Ollama Locally (Free and Offline)
...
2. In Unity, open the training scene and select the `OllamaClient` component:
   - Set **`Llm`** to the exact model name downloaded (e.g., `llama3.2` or `biomistral`).
...
- **Error: "Ollama connection refused" or port 11434 timeout:**
  ...
  - *Solution:* Open a terminal and run `ollama serve`, or verify the model is pulled using `ollama list`. If missing, run `ollama pull llama3.2`.
